...
export const useQuizStore = create<QuizState>((set) => ({
  currentQuestion: 0,
  answers: Array(10).fill(null), // Adjusted to match the number of questions
  score: 0,
  showScore: false,
  questions: [
    {
  ...
  nextQuestion: () =>
    set((state) => {
      ...
      if (isLastQuestion) {
        let score = state.questions.reduce((total, question, index) => {
          return (
            total + (state.answers[index] === question.correctAnswer ? 1 : 0)
          );
        }, 0);
